...
Si $\gamma$ est une courbe de Jordan, on notera $\Int{\gamma}$ et $\Ext{\gamma}$ ses composantes bornées
et non bornées délimitées.
Le théorème suivant vient compléter l'information donnée par le théorème de Jordan
\begin{theorem*}
    Soit $\gamma$ une courbe de Jordan.
    Il existe un homéomorphisme de $\mathbb{R}^2$ dans lui même, tel que l'image de $\Int{\gamma}$ par ce dernier
...
\begin{prop} \label{passDouane}
    Soit $A$ un connexe et $B \subset \mathbb{R}^2$ tel que $A \cap B \neq \varnothing$
    et $A \backslash B \neq \varnothing$.
...
\begin{prop} \label{CCPAouv}
    Soit $V$ un ouvert. Ses composantes connexes par arcs sont ouvertes
\end{prop}
...
\begin{prop} \label{frontCCPAinfront}
...
\begin{proof}
    ...
    Par connexité et la proposition \ref{passDouane}, $A$ rencontre $\operatorname{Fr}(V)$\\
    Par la Proposition \ref{frontCCPAinfront}, $\operatorname{Fr}(V) \subset \operatorname{Fr}(U) \subset \mathbb{R}^2 \backslash U$ car $U$ est ouvert\\
    Donc $A$ rencontre le complémentaire de $U$, absurde par inclusion
    \\~\\
    Donc $A \subset V$
\end{proof}

\begin{prop} \label{CCPAouvRec}
...
\begin{proof}
    ...
    tel que $U \cap V_i \neq \varnothing$\\
    Par la Proposition \ref{CPAinCCPA}, $V_i \subset U$
    \\~\\
...
\begin{tcolorbox}[colback = purple!20!white, colframe = purple!60!white, title = Proposition 7]
    ...
    On écrit $x = \gamma(t_0)$ et $y = \gamma(t_1)$ et on suppose $t_0 \leq t_1$\\
    ...
    Il faut aussi pouvoir contourner un bout de courbe qui touche qu'à un endroit
\end{tcolorbox}
